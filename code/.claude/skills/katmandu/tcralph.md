...
### RuboCop as the growth constraint

The TCR gate isn't just tests — RuboCop is embedded in the loop. It enforces 5-line methods, 100-line classes, and consistent style. This means kenny can't generate bloated code even if he wants to. When a method gets too big, he has to extract. When a class grows past the limit, he has to split.

The result: code grows by adding small files, not by inflating existing ones. As a reviewer, you don't need to think hard — the code is already small, pretty, and simple. You just check that the files make sense, the modules are right, and the naming is clean. RuboCop forces the shape; kenny fills it in.
