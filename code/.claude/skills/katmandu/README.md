...
The loop is also guided by tests from the outside in — the **GOOS** approach (Growing Object-Oriented Software, Guided by Tests) by **Nat Pryce** and **Steve Freeman**. A failing acceptance scenario defines the outer boundary. Implementation works inward: the scenario forces a class, that class calls another, and so on down the layers. Tests at each layer drive the design of the layer beneath.
...
2. **Gherkin** (`/gherkin`) — write acceptance scenarios from those examples
3. **Katmandu** (`/katmandu`) — implement code in small steps to make scenarios pass

Gherkin hands katmandu a failing scenario. Kent breaks it down. Kenny implements each piece. When all tasks are done, control returns to gherkin for verification.
