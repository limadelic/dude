...
describe Dude::Dudes::Inbox do
  ...
  describe '#append' do
    ...
    it 'appends to existing messages' do
      allow(JSON).to receive(:load_file) { [{ 'text' => 'old', 'status' => 'new' }] }
      inbox.append({ 'text' => 'hi' })
      expect(File).to have_received(:write).with(path, '[{"text":"old","status":"new"},{"text":"hi"}]')
    end
  end

  describe '#mark_wip' do
    it 'sets first item status to wip' do
      allow(JSON).to receive(:load_file) { [{ 'text' => 'hi', 'status' => 'new' }] }
      inbox.mark_wip
      expect(File).to have_received(:write).with(path, '[{"text":"hi","status":"wip"}]')
    end
  ...
  describe '#first_new' do
    ...
    it 'returns nil when no new messages' do
      allow(JSON).to receive(:load_file) { [{ 'text' => 'hi', 'status' => 'wip' }] }
