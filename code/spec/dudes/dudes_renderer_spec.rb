...
describe Dude::StatusLine::Dudes do
  ...
  def mock_dude(name:, icon:, messages:, context:, current:, abiding: true)
    d = double(name)
    allow(d).to receive_messages(name: name, icon: icon, messages: messages,
  ...
  let(:dudes_data) do
    [
      mock_dude(name: 'dude', icon: '🎳', messages: 0, context: 25, current: true),
      mock_dude(name: 'rec', icon: '🔴', messages: 3, context: 50, current: false),
      mock_dude(name: 'smith', icon: '🤖', messages: 0, context: 80, current: false)
    ]
  end
  ...
  describe 'Abide watcher status' do
    context 'not abiding' do
      it 'shows ˣ with context color' do
        dudes = [mock_dude(name: 'rec', icon: '🔴', messages: 3, context: 50, current: false, abiding: false)]
        output = out(session, activity, dudes)
        expect(output).to include("\e[38;5;226m🔴", "ˣ")
      end

      it 'shows green ˣ with low context' do
        dudes = [mock_dude(name: 'rec', icon: '🔴', messages: 0, context: 10, current: false, abiding: false)]
        output = out(session, activity, dudes)
        expect(output).to include("\e[32m🔴", "ˣ")
      end

      it 'has background highlight for current not-abiding dude' do
        dudes = [mock_dude(name: 'rec', icon: '🔴', messages: 0, context: 25, current: true, abiding: false)]
        output = out(session, activity, dudes)
        expect(output).to include("\e[42m\e[97m🔴", "ˣ")
      end

      it 'shows red ˣ with high context' do
        dudes = [mock_dude(name: 'rec', icon: '🔴', messages: 0, context: 80, current: false, abiding: false)]
        output = out(session, activity, dudes)
    ...
    context 'abiding' do
      it 'shows message count' do
        dudes = [mock_dude(name: 'rec', icon: '🔴', messages: 3, context: 50, current: false, abiding: true)]
        output = out(session, activity, dudes)
        expect(strip(output)).to match(/🔴 ?³/)
      end
    end
    ...
    context 'context color' do
      { 0 => 'green', 32 => 'green', 33 => 'yellow', 66 => 'yellow', 67 => 'red', 100 => 'red' }.each do |p, c|
        it "writes #{c} at #{p}%" do
          ...
          allow(File).to receive(:write)
          d.write_status
          expect(File).to have_received(:write).with(anything, include("\"color\":\"#{c}\""))
        end
      end
    end
  end
