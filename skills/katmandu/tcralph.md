...
### bob cop as the growth constraint

The TCR gate isn't just tests - bob cop is embedded in the loop. It enforces consistent style and idiomatic code. Kenny can't generate sloppy code even if he wants to. When a function gets too big, he has to extract. When a module grows unwieldy, he has to split.

The result: code grows by adding small modules, not by inflating existing ones. As a reviewer, you don't need to think hard - the code is already small, clean, and idiomatic. You just check that the modules make sense, the naming is clear. Bob cop forces the shape; kenny fills it in.
